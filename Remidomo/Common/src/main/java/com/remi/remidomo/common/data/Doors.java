...
public class Doors {
	...
	public Doors(BaseService service) {
	...
	public void setState(int index, State state, boolean enable_alert, Class notificationActivity) {
		setState(index, state, enable_alert, new Date(), notificationActivity);
	}
	
	public void setState(int index, State state, boolean enable_alert, Date tstamp, Class notificationActivity) {
		states[index] = state;
        ...
        // and if not server
		if ((notificationActivity != null) && enable_alert) {
			if (index == GARAGE) {
				if (states[GARAGE] == State.CLOSED) {
					service.showAlertNotification(service.getString(R.string.garage_closed),
                            BaseService.NotifType.GARAGE, R.drawable.garage_closed, BaseActivity.SWITCHES_VIEW_ID, notificationActivity, tstamp);
				} else if (states[GARAGE] == State.OPENED) {
                    service.showAlertNotification(service.getString(R.string.garage_opened),
                            BaseService.NotifType.GARAGE, R.drawable.garage_opened, BaseActivity.SWITCHES_VIEW_ID, notificationActivity, tstamp);
				}
			}
		}
	}

	public void setFromPushedIntent(Intent intent, Class notificationActivity) {
		...
		setState(Integer.parseInt(index), state, true, tstamp, notificationActivity);
		...
		if (state == State.MOVING) {
            ...
            service.showAlertNotification(service.getString(R.string.garage_moving),
                    BaseService.NotifType.ALERT, R.drawable.garage_moving, BaseActivity.SWITCHES_VIEW_ID, notificationActivity, tstamp);
		}
	}
	
	public synchronized void syncWithServer(int port, String ipAddr, Class notificationActivity) {
		...
		try {
			...
			for (int i=0; i<MAX_DOORS; i++) {
				if (i<current.length()) {
					...
					setState(i, State.valueOf(stateStr), false, notificationActivity);
				}
			}
			...
			service.addLog("Mise à jour des états portes depuis le serveur", BaseService.LogLevel.UPDATE);
			if (service.callback != null) {
				service.callback.updateDoors();
			}
	...
	public void syncWithHardware(xPLMessage msg) {
		...
		if (i == -1) {
			...
			service.addLog("Message portes provenant d'une adresse inconnue: " + address, BaseService.LogLevel.HIGH);
			return;
		}
		...
		if ("normal".equals(command)) {
			...
			service.addLog("Message portes avec commande inconnue :" + command, BaseService.LogLevel.HIGH);
		}
		...
		if (garageStates[0] && garageStates[1]) {
			...
			service.addLog("Anomalie porte de garage: " + garageStates[0] + "/" + garageStates[1], BaseService.LogLevel.HIGH);
		}

		if (newState != states[GARAGE]) {
			Log.i(TAG, "Doors state updated from hardware: " + newState);
			service.addLog("Portes synchronisées avec le matériel", BaseService.LogLevel.UPDATE);

			setState(GARAGE, newState, true, null);
