...
import static com.remi.remidomo.common.data.SensorData.Pair;
...
public class Sensors {
	...
	public Sensors(BaseService service) {
		...
		new Thread(new Runnable() {
        	public void run() {
        		...
        		synchronized (sensors) {
        			...
        			Sensors.this.service.addLog("Lecture terminée", BaseService.LogLevel.UPDATE);
        			...
        			if (allEmpty) {
        				...
        				Intent intent = new Intent(Sensors.this.service, BaseService.class);
                        intent.setAction(BaseService.ACTION_RESTORE_DATA);
    ...
    public void updateData(BaseService service, xPLMessage msg, Class notificationActivity) {
    	updateData(service, msg, SensorData.CompressionType.TIME_BASED, notificationActivity);
    }

    public void updateData(BaseService service, xPLMessage msg, SensorData.CompressionType compress, Class notificationActivity) {
    	...
    	synchronized(sensors) {
    		...
    		checkSensorsConsistency(notificationActivity);
    	}
    }
    ...
    public void syncWithServer(int port, String ipAddr) {
    ...
    public boolean checkSensorsConsistency(Class notificationActivity) {
    	synchronized (sensors) {
    		...
    		for (SensorData sensor: sensors) {
    			...
    			if (maxTime - tstamp > WARN_MISSING_MINS*60*1000) {
    				...
    				if ((service != null) && (!warnedAboutMissingSensor)) {
    					...
    					service.showAlertNotification(msg, BaseService.NotifType.ALERT, R.drawable.temperature2, BaseActivity.TEMP_VIEW_ID, notificationActivity, new Date());
    					service.addLog(msg);
