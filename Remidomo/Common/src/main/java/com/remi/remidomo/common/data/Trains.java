...
public class Trains {
	...
	private JSONObject getStationData(BaseService service, String gare) {
        try {
        	...
        	service.addLog("Erreur URI SNCF: " + e.getLocalizedMessage(), BaseService.LogLevel.HIGH);
        	Log.e(TAG, "Bad URI: " + SNCF_URL+gare);
        } catch (org.apache.http.client.ClientProtocolException e) {
        	service.addLog("Erreur protocole SNCF: " + e.getLocalizedMessage(), BaseService.LogLevel.HIGH);
        	Log.e(TAG, "ClientProtocolException with URI: " + SNCF_URL + gare + ", " + e);
        } catch (java.io.IOException e) {
        	service.addLog("Erreur I/O SNCF: " + e.getLocalizedMessage(), BaseService.LogLevel.HIGH);
        	Log.e(TAG, "IOException with URI: " + SNCF_URL + gare + ", " + e);
        } catch (org.json.JSONException e) {
        	service.addLog("Erreur JSON SNCF", BaseService.LogLevel.HIGH);
	...
	public void updateData(BaseService service) {
		
		SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(service);
		String gare = prefs.getString("gare", Defaults.DEFAULT_GARE);
		...
		if ("GOC".equals(gare)) {
			...
			service.addLog("Pas de trains en commun entre les 2 gares", BaseService.LogLevel.MEDIUM);
			return;
		}
		...
		try {
			...
			if (departs.length() == 0) {
				service.addLog("Infos SNCF vides", BaseService.LogLevel.MEDIUM);
				...
				service.addLog("Mise à jour des données SNCF (" + newTrains.size() + " trains)", BaseService.LogLevel.UPDATE);
