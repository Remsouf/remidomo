...
public class SensorData {
	...
	public SensorData(String name, BaseService service, boolean initFromFile) {
		this.name = name;
		...
		try {
			if (dir == DirType.INTERNAL) {
				...
				if (Environment.MEDIA_MOUNTED.equals(state)) {
					...
					service.addLog("Impossible de lire depuis la carte SD", BaseService.LogLevel.HIGH);
				}
			} else {
				assert(false);
			}
			...
			service.addLog("Pas de données locales pour " + name, BaseService.LogLevel.HIGH);
		} catch (java.lang.IllegalArgumentException e) {
			Log.e(TAG, "Error parsing sensor file for sensor " + name + " (IllegalArgument)");
			service.addLog("Erreur de lecture pour " + name, BaseService.LogLevel.HIGH);
		...
		try {
			if (dir == DirType.INTERNAL) {
				...
				if (Environment.MEDIA_MOUNTED.equals(state)) {
				    ...
				    service.addLog("Impossible d'écrire sur la carte SD", BaseService.LogLevel.HIGH);
				}
			} else {
				assert(false);
			}
			...
			service.addLog("Impossible d'écrire le fichier de données pour " + this.name, BaseService.LogLevel.HIGH);
		} finally {
			if (fos != null) {
				try {
					fos.close();
	...
	public synchronized void addValue(Date tstamp, float value, CompressionType compress) {
		...
		if (service != null) {
			...
			int maxHistory = prefs.getInt("loglimit", Defaults.DEFAULT_LOGLIMIT);
